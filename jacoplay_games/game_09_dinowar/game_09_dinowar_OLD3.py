...
def resolve_ability(
    ...
    targets_idx:
      - None per SELF/ALL (calcolato internamente)
      - lista indici target per ONE/ALL
    Ritorna (applied, game_over_result) dove game_over_result è "MENU" se finisce.
    """
    caster = caster_state.field[caster_idx]
    ok, msg = can_cast_ability(caster)
    if not ok:
        return False, msg  # msg come testo toast

    ability = caster.base.ability
    a_type = ability.type.upper()
    a_targets = ability.targets.upper()

    # determina lista targets (CardInstance references) e anche i riferimenti agli stati
    target_cards: List[CardInstance] = []
    target_state = caster_state  # default
    ...
    if a_targets == "SELF":
        ...
        if a_type == "ATTACK":
            ...
            target_cards = list(other_state.field)
            idxs = list(range(len(other_state.field)))
        else:
            target_state = caster_state
            target_cards = list(caster_state.field)
            idxs = list(range(len(caster_state.field)))
    elif a_targets == "ONE":
        ...
        if a_type == "ATTACK":
            target_state = other_state
            if idxs[0] < 0 or idxs[0] >= len(other_state.field):
                return False, "Bersaglio non valido"
            target_cards = [other_state.field[idxs[0]]]
        else:
            target_state = caster_state
            if idxs[0] < 0 or idxs[0] >= len(caster_state.field):
                return False, "Bersaglio non valido"
            target_cards = [caster_state.field[idxs[0]]]
    else:
    ...
    # consuma energia e marca abilità usata
    caster_state.energy -= cost
    caster.ability_used_this_turn = True

    # applica effetto
    if a_type == "HEAL":
        apply_heal(target_cards, int(ability.point_per_target))
    elif a_type == "SPEED":
        apply_speed(target_cards, int(ability.point_per_target))
        # aggiorna glow per utente se serve
        if caster_is_user:
            for inst in caster_state.field:
                inst.glow = should_glow_for_attack(inst) and (not inst.selected)
    elif a_type == "ATTACK":
        ...
        # Per il rendering: ricostruiamo sempre chi è USER e chi è AI
        ...
        # Determina dove stanno i target (alto = AI, basso = USER)
        # Per ATTACK: i target sono sempre other_state.
        target_is_ai = caster_is_user  # se caster è utente, colpisce AI (alto); viceversa colpisce utente (basso)

        # Snapshot dei riferimenti (così se una carta muore e viene rimossa, troviamo comunque gli altri target)
        targets_snapshot = list(target_cards)

        for target_obj in targets_snapshot:
            # trova l'indice corrente del target nel field (può cambiare se vengono rimosse carte)
            current_idx = None
            ...
            # applica danno (considera eventuale shield)
            died = apply_ability_attack_damage(points, target_obj)

            if died:
                # suono morte PRIMA della rimozione
                audio.play_sfx(SFX_DEAD, volume=0.9)
                pygame.time.delay(120)
                # rimuovi (se ancora presente)
                for j, inst in enumerate(target_state.field):
                    if inst is target_obj:
                        target_state.field.pop(j)
                        break

    else:
        return False, "Abilità non implementata"

    # check game over (energie non contano -> la tua funzione già aggiornata)
    res = check_and_handle_game_over(
        scaler=scaler, cache=cache, audio=audio, fonts=fonts,
        user_state=caster_state if caster_is_user else other_state,
        ai_state=other_state if caster_is_user else caster_state,
        clock=clock
    )
    if res is not None:
        return True, "MENU"
...
def start_match(
    ...
    while True:
        ...
        # ===== Skill button (latched) =====
        if mouse_up and user_turn and btn_skill.is_clicked(mouse_logical, True):
            # Toggle OFF if already active
            if skill_active:
                skill_active = False
                skill_waiting_target = False
                btn_skill.set_forced(False)
            else:
                if selected_attacker_idx is None or selected_attacker_idx >= len(user_state.field):
                    toast = Toast("Seleziona una carta per usare l'abilità", 1500)
                else:
                    ...
                    if not ok:
                        ...
                        # latch ON
                        skill_active = True
                        btn_skill.set_forced(True)

                        # SELF / ALL -> resolve immediately and turn OFF
                        if ability.targets.upper() in ("SELF", "ALL"):
                            ...
                            caster.glow = should_glow_for_attack(caster)
                            selected_attacker_idx = None
                        else:
                            # ONE -> wait for target click
                            skill_waiting_target = True
        ...
        if user_turn:
            ...
            if mouse_up:
                ...
                # ===== Skill target selection (ONE) =====
                if skill_active and skill_waiting_target and selected_attacker_idx is not None:
                    caster = user_state.field[selected_attacker_idx]
                    ab = caster.base.ability
                    # Click on caster again cancels the skill selection
                    if idx_u is not None and idx_u == selected_attacker_idx:
                        ...
                        skill_waiting_target = False
                        btn_skill.set_forced(False)
                        # skip attack handling
                        continue

                    target_idxs: Optional[List[int]] = None
                    if ab.targets.upper() == "ONE":
                        if ab.type.upper() == "ATTACK" and idx_a is not None:
                            target_idxs = [idx_a]
                        elif ab.type.upper() in ("HEAL", "SPEED") and idx_u is not None:
                            target_idxs = [idx_u]
