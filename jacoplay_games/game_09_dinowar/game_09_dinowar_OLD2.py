...
def ability_cost(ability: AbilityDef, n_targets: int) -> int:
    return max(0, int(ability.point_per_target)) * max(0, int(n_targets))
...
def resolve_ability(
    ...
    if a_type == "HEAL":
        ...
        # animazione: riusiamo colpo per 1 target; per ALL facciamo una breve sequenza
        # suono: usiamo gli stessi suoni attacco base in base alla fazione del caster
        if caster.base.faction == FACTION_CARN:
            audio.play_sfx(SFX_CARN_ATTACK, volume=0.9)
        else:
            audio.play_sfx(SFX_HERB_ATTACK, volume=0.9)

        # Applica danni e rimuovi morti (in ordine inverso per sicurezza)
        dead_indices: List[int] = []
        for i, t in zip(idxs, target_cards):
            if apply_ability_attack_damage(int(ability.point_per_target), t):
                dead_indices.append(i)

        # Suono morte e rimozione (prima il suono)
        if dead_indices:
            audio.play_sfx(SFX_DEAD, volume=0.9)
            pygame.time.delay(120)
            for i in sorted(dead_indices, reverse=True):
                if 0 <= i < len(target_state.field):
                    target_state.field.pop(i)
    else:
